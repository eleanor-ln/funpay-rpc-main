...
export class SettingsManager {
  ...
  private html(): string {
    return `<!doctype html><html><head><meta charset="UTF-8"><style>
      :root { color-scheme: dark; --text: #f4f4f4; --muted: #9b9b9b; --panel: rgba(12, 12, 13, .95); --card: rgba(23, 23, 24, .94); --border: rgba(255, 255, 255, .1); --track: #101a2b; --track-on: #1d3658; --accent: #8dbdf2; }
      * { box-sizing: border-box; } html { background: #0b0b0c; } body { margin: 0; min-height: 100%; padding: 22px; overflow-y: auto; background: var(--panel); backdrop-filter: blur(26px) saturate(110%); -webkit-backdrop-filter: blur(26px) saturate(110%); color: var(--text); font: 14px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; opacity: 0; transform: translateX(16px); transition: opacity .2s,transform .2s; border-left: 1px solid rgba(255, 255, 255, .1); } body.visible { opacity: 1; transform: translateX(0); }
