...
(function () {
    ...
    function installHeaderButton() {
        ...
        if (!controls) return;
        if (controls.querySelector('.fxn-search-toggle')) return;

        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'btn btn-info-icon btn-info-sm btn-gray fxn-search-toggle';
    ...
    function toggleInChatSearch() {
        ...
        bar.className = 'fxn-inchat-search';
        bar.innerHTML = `
            <span class="material-symbols-rounded fxn-ics-ico">search</span>
            <input type="text" class="fxn-ics-input" placeholder="Поиск по этому чату...">
            <span class="fxn-ics-count"></span>
            <button class="fxn-ics-nav" data-d="-1" title="Предыдущее"><span class="material-symbols-rounded">keyboard_arrow_up</span></button>
            <button class="fxn-ics-nav" data-d="1" title="Следующее"><span class="material-symbols-rounded">keyboard_arrow_down</span></button>
            <button class="fxn-ics-close" title="Закрыть"><span class="material-symbols-rounded">close</span></button>`;
        header.insertAdjacentElement('afterend', bar);
        requestAnimationFrame(() => bar.classList.add('open'));

        const input = bar.querySelector('.fxn-ics-input');
        input.addEventListener('input', debounce(() => runInChatSearch(input.value), 160));
        input.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') { e.preventDefault(); stepMatch(e.shiftKey ? -1 : 1); }
            if (e.key === 'Escape') closeInChatSearch();
        });
        bar.querySelectorAll('.fxn-ics-nav').forEach(b =>
            b.addEventListener('click', () => stepMatch(+b.dataset.d)));
        bar.querySelector('.fxn-ics-close').addEventListener('click', closeInChatSearch);
        input.focus();
        document.querySelector('.fxn-search-toggle')?.classList.add('active');
    }
    ...
    function runInChatSearch(q) {
        ...
        const bar = document.querySelector('.fxn-inchat-search');
        const countEl = bar && bar.querySelector('.fxn-ics-count');
        ...
        getMessageNodes().forEach(item => {
            ...
            if (txt && norm(txt).includes(query)) {
                item.classList.add('fxn-ics-hit');
    ...
    function focusMatch() {
        const bar = document.querySelector('.fxn-inchat-search');
        const countEl = bar && bar.querySelector('.fxn-ics-count');
        inChatMatches.forEach(m => m.classList.remove('fxn-ics-current'));
        const cur = inChatMatches[inChatIndex];
        if (!cur) return;
        cur.classList.add('fxn-ics-current');
        cur.scrollIntoView({ behavior: 'smooth', block: 'center' });
        if (countEl) countEl.textContent = `${inChatIndex + 1} / ${inChatMatches.length}`;
    }
    function clearInChatHighlights() {
        document.querySelectorAll('.fxn-ics-hit,.fxn-ics-current')
            .forEach(m => m.classList.remove('fxn-ics-hit', 'fxn-ics-current'));
    }
    function closeInChatSearch() {
        ...
        document.querySelector('.fxn-search-toggle')?.classList.remove('active');
    }
