(function initNewChatStarter() {
    ...
    const STYLE_ID = 'fxn-new-chat-css';
    const ROOT_ID = 'fxn-new-chat';
    ...
    function injectStyles() {
        ...
        #${ROOT_ID} .fxn-nc-start{cursor:pointer;}
        #${ROOT_ID} .fxn-nc-form{
            width:100%;display:none;flex-direction:column;gap:10px;
            animation:fxnNcIn .18s ease;
        }
        #${ROOT_ID}.open .fxn-nc-form{display:flex;}
        #${ROOT_ID}.open .fxn-nc-start{display:none;}
        @keyframes fxnNcIn{from{opacity:0;transform:translateY(6px);}to{opacity:1;transform:translateY(0);}}
        #${ROOT_ID} .fxn-nc-inputwrap{position:relative;}
        #${ROOT_ID} .fxn-nc-input{padding-right:34px;text-align:left;}
        #${ROOT_ID} .fxn-nc-spin{
            ...
            animation:fxnNcSpin .7s linear infinite;pointer-events:none;
        }
        #${ROOT_ID}.loading .fxn-nc-spin{display:block;}
        @keyframes fxnNcSpin{to{transform:rotate(360deg);}}
        #${ROOT_ID} .fxn-nc-result{
            border:1px solid var(--gray-300, rgba(128,128,128,.25));
            border-radius:8px;overflow:hidden;display:none;
        }
        #${ROOT_ID} .fxn-nc-result.show{display:block;}
        #${ROOT_ID} .fxn-nc-card{
            display:flex;align-items:center;gap:11px;
            padding:11px 13px;cursor:pointer;text-decoration:none;
            color:inherit;transition:background .12s ease;
        }
        #${ROOT_ID} .fxn-nc-card:hover{background:var(--gray-200, rgba(128,128,128,.12));}
        #${ROOT_ID} .fxn-nc-ava{
            width:42px;height:42px;border-radius:50%;flex-shrink:0;
            object-fit:cover;background:var(--gray-200, rgba(128,128,128,.15));
        }
        #${ROOT_ID} .fxn-nc-info{flex:1;min-width:0;text-align:left;}
        #${ROOT_ID} .fxn-nc-uname{
            font-size:14px;font-weight:600;
            white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
        }
        #${ROOT_ID} .fxn-nc-uid{font-size:12px;opacity:.5;margin-top:1px;}
        #${ROOT_ID} .fxn-nc-go{
        ...
        #${ROOT_ID} .fxn-nc-msg.show{display:block;}
        #${ROOT_ID} .fxn-nc-cancel{
            background:none;border:none;cursor:pointer;
            font-size:12px;opacity:.5;padding:2px;align-self:center;
        }
        #${ROOT_ID} .fxn-nc-cancel:hover{opacity:.85;text-decoration:underline;}
        `;
    ...
    function build() {
        ...
        root.id = ROOT_ID;
        root.innerHTML = `
            <button type="button" class="fxn-nc-start btn btn-default">Начать новый чат</button>
            <div class="fxn-nc-form">
                <div class="fxn-nc-inputwrap">
                    <input type="text" class="fxn-nc-input form-control" placeholder="Введите ник пользователя" autocomplete="off" spellcheck="false">
                    <span class="fxn-nc-spin"></span>
                </div>
                <div class="fxn-nc-msg"></div>
                <div class="fxn-nc-result"></div>
                <button type="button" class="fxn-nc-cancel">отмена</button>
            </div>`;

        const startBtn = root.querySelector('.fxn-nc-start');
        const input = root.querySelector('.fxn-nc-input');
        const cancel = root.querySelector('.fxn-nc-cancel');
    ...
    function showMsg(root, html) {
        const el = root.querySelector('.fxn-nc-msg');
        if (!el) return;
    ...
    function hideResult(root) {
        const el = root.querySelector('.fxn-nc-result');
    ...
    async function doResolve(root, username) {
        if (busy) return;
        if (username === lastQuery && root.querySelector('.fxn-nc-result.show')) return;
        lastQuery = username;
        busy = true;
    ...
    function renderResult(root, data, avatar) {
        ...
        const el = root.querySelector('.fxn-nc-result');
        el.innerHTML = `
            <a class="fxn-nc-card" href="${chatUrl}">
                <img class="fxn-nc-ava" src="${esc(ava)}" alt="">
                <div class="fxn-nc-info">
                    <div class="fxn-nc-uname">${esc(uname)}</div>
                    <div class="fxn-nc-uid">ID ${esc(theirId)}</div>
                </div>
                <span class="fxn-nc-go">Открыть чат →</span>
            </a>`;
        el.querySelector('.fxn-nc-ava')?.addEventListener('error', function() { this.src = 'https://funpay.com/img/layout/avatar.png'; });
        el.classList.add('show');
    }
