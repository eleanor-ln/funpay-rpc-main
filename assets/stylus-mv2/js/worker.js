...
(() => {
  ...
  function getMetaComment(str, action) {
    ...
    if (action === "del") {
      var h;
      res = a && (h = b) != null && h[1] ? str.slice(0, a.index) + str.slice(i) : str;
  ...
  function extractSections(code, styleId, metaStr, strict) {
    ...
    parser.addListener("startdocument", e => {
      ...
      const lastCmt = ((h = e.start.comment) == null ? void 0 : h.text) || "";
      const section = {
        code: "",
        start: e.brace.offset + 1
      };
      let outerText = code.slice(lastSection.start, e.offset);
      if (lastCmt && (!(metaStr != null ? metaStr : metaStr = getMetaComment(code)) || !lastCmt.includes(metaStr))) {
        section.code = lastCmt + "\n";
        outerText = outerText.slice(0, -lastCmt.length);
      }
      outerText = outerText.replace(metaStr != null ? metaStr : metaStr = getMetaComment(code), "").trim();
      if (outerText) {
  ...
  function parseJSON(state) {
    ...
    try {
      ...
      h.message = `Invalid JSON: ${h.message}`;
      throw h;
    }
    state.index = pos;
  }
  ...
  function validateURL(state) {
    let url;
    try {
      url = new h(state.value);
    } catch (h) {
      h.args = [ state.value ];
      h.index = state.valueIndex;
      throw h;
    }
        ...
        for (;match = re.exec(text); ) {
          ...
          collectErrors(() => {
            try {
              ...
              h.index === void 0 && (h.index = state.index);
              throw h;
            }
  ...
  const LintWorkerAPI = {
    ...
    async stylelint(code, config, mode) {
      var h;
      stylelint || (stylelint = loadStylelint(mode));
      ...
      const messages = ((h = res._postcssResult) == null ? void 0 : h.messages) || res.warnings;
  ...
  const ruleRetriever = {
    ...
    stylelint() {
      stylelint || (stylelint = loadStylelint());
      const options = {};
  ...
  function extractSectionsFromLess({css}, docs, metaStr, sections) {
    ...
    for (let [cmt, prelude, body, start, end] of docs) {
      ...
      if (prelude && (prelude = Array.isArray(v = prelude.value) ? v : [ prelude ])) for (const node of prelude) {
        var h;
        let k, quote;
        if (typeof (v = node.value) != "string" && (k = node.name || node.type) && (k = FROM_CSS[k.toLowerCase()]) && (v || (v = (h = node.args) == null ? void 0 : h[0])) && typeof (({quote} = v), 
        v = v.value) == "string") {
          var I;
          (sec[I = k] || (sec[I] = [])).push(quote ? v.replace(/\\\\/g, "\\") : v);
        }
      }
      sections.push(sec);
      prevEnd = end;
  ...
  function extractSectionsFromStylus(block) {
    ...
    for (const node of block.nodes) if (!((v = node.str) && v !== metaStrTmp && v.charCodeAt(0) === 47 && (cmt = v) || varsUsed && !sepSkipped && (node.type !== varsSep || (sepSkipped = !0)) || node.suppress)) if (node.type === "-moz-document") {
      ...
      for (const seg of node.segments) {
        var h;
        (k = FROM_CSS[seg.name.toLowerCase()]) && (v = seg.args.first) && (sec[h = k] || (sec[h] = [])).push((v.val || `${v}`).replace(/\\\\/g, "\\"));
      }
  ...
  Object.assign(COMMANDS, {
    compileUsercss: async (code, preprocessor, vars, styleId, strict) => {
      ...
      if (fn && (code = fn(code, metaStr, vars, sections, log, warn)) && code.then) {
        var h;
        const me = builderChain = ((h = builderChain) == null ? void 0 : h.catch(() => {}).then(code)) || code;
        code = await builderChain;
        builderChain === me && (builderChain = null);
      }
      sections || (sections = extractSections(code, styleId, metaStr, strict));
