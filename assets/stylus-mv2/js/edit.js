...
(self.webpackChunkStylus = self.webpackChunkStylus || []).push([ [ "edit" ], {
  4160(_, ee, oe) {
    ee.addEditorCommands = () => {
      for (const cmd of [ "nextEditor", "prevEditor", "save", "toggleStyle" ]) cmCommands[cmd] = he.default[cmd];
    };
    var le = oe(2715);
    var ae = oe(6161);
    oe(4318);
    ...
    const cmFactory = {
      create(place, options, finishInit) {
        finishInit && ((options != null ? options : options = {}).finishInit = finishInit);
        const cm = le.CodeMirror(place, options);
    ...
    const onCmFocus = cm => {
      ye.rerouteHotkeys.toggle(!1);
      cm.display.wrapper.classList.add("CodeMirror-active");
      cm.lastActive = Date.now();
    };
    const onCmBlur = cm => {
      setTimeout(() => {
        ye.rerouteHotkeys.toggle(!0);
        const {wrapper} = cm.display;
        wrapper.classList.toggle("CodeMirror-active", wrapper.contains(document.activeElement));
      });
    };
    const onCmBeforeChange = (cm, {text}) => {
      const max = Math.max(cm.options.maxHighlightLength, 1e5);
      for (const line of text) if (line.length > max) {
        const el = document.querySelector("#lineWrapping-label + a");
        el.hidden = !1;
        el.title = (line.length / 1e3 | 0) + "k long line detected, text wrapping was disabled to ensure the browser doesn't crash or hang";
    ...
    const maybeImportOnPaste = (cm, evt) => {
      ...
      /^\s*{\s*"doc"\s*:\s*{\s*"/.test(text) && (v = fe.tryJSONparse(text)) && pe.styleJSONseemsValid(v = v.doc) ? (ucText = v.sourceCode) || (text = pe.styleToCss(v)) : v = null;
      if (he.default.isUsercss) {
        if (v) {
          evt.preventDefault();
        ...
        var _;
        he.default.importOnPaste(cm, evt, ucText || text, !ucText && ((_ = v) == null ? void 0 : _.sections));
      }
    };
    le.CodeMirror.defineInitHook(cm => {
      ...
      cm.options[kLineWrapping] && cm.on("beforeChange", onCmBeforeChange);
    });
    const prefToCmOpt = k => k.startsWith("editor.") && k.slice(7);
    const prefKeys = ue.knownKeys.filter(k => k !== "editor.colorpicker" && k !== "editor.arrowKeysTraverse" && prefToCmOpt(k) in le.CodeMirror.defaults);
    const {insertTab, insertSoftTab} = cmCommands;
    (async () => {
      await ue.ready;
      for (const key of prefKeys) cmDefaults[prefToCmOpt(key)] = ue.__values[key];
      for (const [key, fn] of Object.entries({
        ...
        le.CodeMirror.defineOption(prefToCmOpt(key), ue.__values[key], fn);
        prefKeys.push(key);
      }
    })();
    ue.subscribe(prefKeys, async (key, val) => {
    ...
    function selectTokenOnDoubleclick(cm, {ch, line}) {
      ...
      let a = ch;
      let b, rx;
      rxsrcUniBody != null || (rxsrcUniBody = ae.rxUniBody.source);
      ...
      if (!style || style && (rx = /^(?:comment|string|(uso-variable))/.exec(style))) {
        var _;
        rx = (_ = rx) != null && _[1] ? rxWordDashed != null ? rxWordDashed : rxWordDashed = RegExp(rxsrcUniBody.slice(0, -1), "yu") : rxWord != null ? rxWord : rxWord = RegExp("[" + rxsrcUniBody.slice(2, -1), "yu");
        for (;a && (rx.lastIndex = a - 1, rx.test(text)); ) --a;
        b = getB();
        for (let retry = 0; a === ch && b === ch && retry < 2; ++retry) {
          rx = retry ? /\s/y : rxNonIdent1 != null ? rxNonIdent1 : rxNonIdent1 = RegExp(`[^${rxsrcUniBody.slice(2, -2)}\\s]`, "yu");
          for (;a && (rx.lastIndex = a - 1, rx.test(text)); ) --a;
          rx = retry ? /\s*/y : rxNonIdentMany != null ? rxNonIdentMany : rxNonIdentMany = RegExp(rxNonIdent1.source + "*", "yu");
          rx.lastIndex = ch;
        ...
        a = i > 0 ? styles[i] : 0;
        b || (a += text.slice(a, b = getB()).search(rxQualifier != null ? rxQualifier : rxQualifier = RegExp(`[#.]?${rxsrcUniBody}%?$`, "u")));
  ...
  6767(_, ee, oe) {
    ...
    var prefs = oe(2956);
    var util = oe(2076);
    var compact_header = oe(4e3);
    var live_preview = oe(4614);
    var windowed_mode = oe(3160);
    ...
    const editor = self.editor = {
      ...
      style,
      toc,
      applyScrollInfo(cm, si = (_ => (_ = scrollInfo.cms) == null ? void 0 : _[0])()) {
        if (si && si.sel) try {
          const bmOpts = {
            sublimeBookmark: !0,
            clearWhenEmpty: !1
      ...
      updateToc(added) {
        ...
        if (first >= 0 && (!added.focus || !elFirst)) for (let el = elFirst, i = first; i < sections.length; i++) {
          ...
          el = (_ = el) == null ? void 0 : _.nextElementSibling;
        }
  ...
  9487(_, ee, oe) {
    ...
    var dom = oe(9298);
    var prefs = oe(2956);
    var codemirror_factory = oe(4160);
    var compact_header = oe(4e3);
    var editor = oe(6767);
    var dom_prefs = oe(113);
    var dom_util = oe(4777);
    var util = oe(2076);
    var edit_util = oe(4165);
    let cssBeautifyMod;
    ...
    async function beautify(scope, ui = !0) {
      cssBeautifyMod || (cssBeautifyMod = (await oe.e("vendor-overwrites_beautify_beautify-css-mod_js").then(oe.bind(oe, 6520))).default);
    ...
    async function showLintConfig() {
      RULES[curLinter] || (RULES[curLinter] = await edit_util.worker.getRules(curLinter));
      await oe.e("jsonlint").then(oe.bind(oe, 6961));
    ...
    function hint(cm) {
      ...
      let words = [];
      if (depth === 1 && isStylelint) words = quoted ? [ "rules" ] : []; else if ((depth === 1 || depth === 2) && type && type.includes("property")) words = ruleIds; else if (depth === 2 || depth === 3 && lexical.type === "]") {
        var _;
        words = quoted ? ruleIds.includes(prevWord) && ((_ = options[prevWord]) == null ? void 0 : _[0]) || [] : [ "true", "false", "null" ];
      } else if (depth === 4 && prevWord === "severity") words = [ "error", "warning" ]; else if (depth === 4) words = [ "ignore", "ignoreAtRules", "except", "severity" ]; else if (depth === 5 && lexical.type === "]" && quoted) {
        var ee;
        for (;prevWord && !ruleIds.includes(prevWord); ) prevWord = (search.find(!0) || [])[1];
        words = ((ee = options[prevWord]) == null ? void 0 : ee.slice(-1)[0]) || ruleIds;
      }
      return {
    ...
    async function showLinterErrorMessage(title, contents) {
      ...
      (_ = popup) == null || (_ = _.codebox) == null || _.focus();
    }
    ...
    function handleExternalUpdate(style, reason, editorId) {
      if (reason !== "editPreview" && reason !== "editPreviewEnd" && (reason !== "editSave" || editor.default.msg.editorId !== editorId)) if (reason !== "toggle") {
        (replaceQueue != null ? replaceQueue : replaceQueue = []).push([ style, reason ]);
        ...
        (_ = editor.default.updateMeta) == null || _.call(editor.default);
      }
    }
    ...
    class EditorSection {
      ...
      create(inView) {
        ...
        const cm = el.CodeMirror = codemirror_factory.default.create(wrapper => {
          ...
          value: code
        }, _ => editor.default.applyScrollInfo(_, si));
      ...
      getModel() {
        ...
        for (const {type, value} of this.targets) {
          var _, ee;
          type && ((ee = res[_ = style_util.FROM_CSS[type]]) != null ? ee : res[_] = []).push(value);
        }
        return res;
      }
      toggle(restore) {
      ...
      updateTocEntry(origin, sec = this) {
        ...
        if (!te.label) {
          ...
          const target = first != null && first.type ? first.value : null;
          if (te.target !== target) {
            te.target = target;
            changed = !0;
          }
      ...
      handleEvent(evt) {
        ...
        const trg = !actionFn && ((_ = el.closest(C_ITEM)) == null ? void 0 : _.me);
        let tmp;
        switch (evt.type) {
         case "click":
    ...
    function SectionsEditor() {
      ...
      container.moveBefore || (container.moveBefore = container.insertBefore);
        ...
        async function doImport({replaceOldStyle = !1}) {
          ...
          try {
            ...
            if (!meta.match(/[\r\n]\s*@preprocessor\s+\S/) || await dom_util.messageBox.alert(util.t("importPreprocessor"))) {
              let name;
              newSections || (newSections = await edit_util.worker.extractSections(code));
      ...
      async function initSections(src, {focusOn = 0, replace = !1, keepDirty = !1, si = editor.scrollInfo} = {}) {
        ...
        si && !si.cms.every(cm => !(cm != null && cm.height)) || requestAnimationFrame(fitToAvailableSpace);
      ...
      function insertSectionAfter(init, base, forceRefresh, si) {
        ...
        forceRefresh || (forceRefresh = base);
      ...
      function moveSection(dir, section) {
        ...
        if (index !== (dir < 0 ? 0 : sections.length - 1)) {
          container.moveBefore(section.el, (_ = sections[index + (dir < 0 ? -1 : 2)]) == null ? void 0 : _.el);
    ...
    function MozSectionWidget(cm, finder = MozSectionFinder(cm)) {
      ...
      function renderFunc(func, old = {}) {
        ...
        const el = ((_ = old.item) == null ? void 0 : _[KEY]) || tplAppliesToItem.cloneNode(!0);
    ...
    function SourceEditor() {
      ...
      const cm = codemirror_factory.default.create(document.querySelector("#sections").appendChild(dom.$create(".single-editor")), {
        mode: style_util.getPreprocessorMode(style.usercssData || (style.usercssData = TEMPLATE_DATA)),
        value: initialCode
      }, me => {
      ...
      Object.assign(editor.default, {
        ...
        async saveImpl() {
          ...
          try {
            ...
            dom_error.elError == null || dom_error.elError.remove();
          } catch (_) {
            showError(_);
          }
          saving = !1;
      ...
      async function replaceStyle(newStyle, draft) {
        ...
        if (draft || await dom_util.messageBox.confirm(util.t("styleUpdateDiscardChanges"))) {
          newStyle.usercssData || (newStyle.usercssData = await metaCompiler(code, {}, cm, !0));
          editor.default.useSavedStyle(newStyle);
          if (!sameCode) {
      ...
      function showError(err) {
        ...
        pos || (pos = ((ee = (_ = err).line) != null ? ee : _.line = err.lineno) && ((le = (oe = err).col) != null ? le : oe.col = err.column) ? {
          line: line = err.line - 1,
          ch: ch = err.col - 1
        } : (ae = err.index) != null ? ae : err.offset);
      ...
      function onCursorActivity() {
        if (prevSel !== cm.doc.sel) {
          prevSel = cm.doc.sel;
          updateTocFocusPending != null || (updateTocFocusPending = Promise.resolve().then(updateTocFocus));
        }
      }
      ...
      function updateTocFocus() {
        ...
        let a = 0;
        let b = end--;
        let c = pos.line && Math.min((_ = toc.i) != null ? _ : a + b >> 1, end);
        let c0, sec;
        for (;a < b && c0 !== c; ) {
          sec = mozSections[c];
        ...
        (ee = toc.el.$("." + toc.cls)) == null || ee.classList.remove(toc.cls);
        toc.i = null;
      }
    }
    oe(7531);
    ...
    src_cm.CodeMirror.commands.colorpicker = cm => {
      var _;
      return (_ = cm.state.colorpicker) == null ? void 0 : _.openPopup();
    };
    ...
    oe(3160);
    (async () => {
      load_style.loading && await load_style.loading;
  ...
  4614(_, ee, oe) {
    ...
    ae.subscribe("editor.livePreview", (key, value, init) => {
      enabled = value;
      init || (value ? livePreview() : port && (port = port.disconnect()));
    }, !0);
  ...
  6421(_, ee, oe) {
    ...
    let id = +pe.urlParams.get("id");
    const loading = ae.clientData.then(function({style, si, theme, themeText, ...props}) {
      Object.assign(fe.default, props);
      Object.assign(fe.default.style, style || (style = makeNewStyleObj()));
      Object.assign(fe.scrollInfo, si);
      fe.default.updateClass();
      fe.default.updateTitle(!1);
      document.getElementById("toc-title").dataset.num = style.sections.length;
      document.getElementById("testRE").hidden = !style.sections.some(({regexps}) => regexps == null ? void 0 : regexps.length);
      pe.sessionStore.justEditedStyleId = id || "";
      if (id === null) {
        pe.urlParams.delete("id");
        const str = `${pe.urlParams}`;
        history.replaceState({}, "", location.pathname + (str ? "?" : "") + str);
      }
      le.loadCmTheme(theme, themeText);
    });
    oe.e("edit-lazy").then(oe.bind(oe, 6932));
    function makeNewStyleObj() {
      id = null;
      const prefix = pe.tryURL(pe.urlParams.get("url-prefix"));
      const name = pe.urlParams.get("name") || prefix.hostname;
      const p = prefix.pathname || "/";
      let section;
      for (let [k, v] of pe.urlParams) (k = ue.FROM_CSS[k]) && ((section != null ? section : section = {})[k] = [ v ]);
      ...
      return {
        id,
        enabled: !0,
        name: name ? name + (p === "/" ? "" : pe.clipString(p.replace(/\.(html?|aspx?|cgi|php)$/, ""))) : pe.urlParams.get("domain") || "?",
        sections: [ section ]
      };
    }
    ee.loading = loading;
  ...
  3160(_, ee, oe) {
    ...
    function EmbeddedPopup() {
      ...
      function removePopup() {
        ...
        xo.disconnect();
        mo = xo = frame = null;
        (_ = document.getElementById(ID)) == null || _.remove();
    ...
    if (util_webext.browserWindows) {
      (async () => {
        ...
        isWindowed ? EmbeddedPopup() : isWindowed = history.length === 1 && (await prefs.ready, 
      ...
      pos && pos.left != null && util_webext.browserWindows.update(util_webext.browserWindows.WINDOW_ID_CURRENT, pos);
    }
    ...
    async function onTabAttached(tabId, info) {
      ...
      openEditInWindow && ua.FIREFOX && util_webext.browserWindows.update(info.newWindowId, prefs.__values.windowPosition);
      prefs.set("openEditInWindow", openEditInWindow);
    }
  }
}, _ => {
  _.O(0, [ "color", "codemirror", "dlg_config-dialog_css-css_dom-error_css-css_global-dark_css-css_global_css-css_onoffswitch_cs-1e56de" ], () => _(_.s = 9487));
  _.O();
} ]);