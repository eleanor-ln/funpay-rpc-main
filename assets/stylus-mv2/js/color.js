  ...
  3696(_, ee, oe) {
    ...
    class Color {
      ...
      static parse(str, len = typeof str == "string" && (str = str.trim()).length, hex) {
        ...
        v = str;
        str = str.toLowerCase();
        if (hex != null ? hex : str.charCodeAt(0) === 35) {
    ...
    ee.default = function(cm) {
      ...
      const PUBLIC_API = {
        ...
        show: opt => {
          ...
          $root.className = [ ...$root.classList ].filter(c => !c.startsWith(`${CSS_PREFIX}theme-`)).concat(CSS_PREFIX + "theme-" + (opt.theme === "dark" || opt.theme === "light" ? opt.theme : guessTheme())).join(" ");
      ...
      function init() {
        ...
        const makeNum = (type, name, channel, channelName, props, min, max) => $([ "input-field", `${name}-${channelName}` ], [ ($inputs[type] || ($inputs[type] = {}))[channel] = $("input", props || {
          tag: "input",
          type: "number",
          min,
          max,
      ...
      function setFromColor(color) {
        typeof color == "string" && (color = le.default.parse(color) || computeColor(color));
        color || (color = le.default.parse("#f00"));
      ...
      function guessTheme() {
        var _;
        const el = options.guessBrightness || cm && (((_ = cm.display.renderedView) == null || (_ = _[0]) == null ? void 0 : _.text) || cm.display.lineDiv);
        return (0, ue.default)(el, {
          bg: "backgroundColor"
        }).bgLuma < .5 ? "dark" : "light";
      }
  ...
  7531(_, ee, oe) {
    ...
    class ColorSwatcher {
      constructor(cm, options = {}) {
        ...
        this.popup = (0, fe.default)(cm);
        if (RX_UNSUPPORTED === void 0 && (ue.CHROME < 125 || ue.FIREFOX < 128)) {
          RX_UNSUPPORTED = [ [ "#abcd", "#(.{4}){1,2}$" ], [ "hwb(1 0% 0%)", "^hwb\\(" ], [ "rgb(1e2,0,0)", "\\de" ], [ "rgb(1.5,0,0)", String.raw`^rgba?\((([^,]+,){0,2}[^,]*\.|(\s*\S+\s+){0,2}\S*\.)` ], [ "rgb(1,2,3,.5)", "[^a]\\(([^,]+,){3}" ], [ "rgb(1,2,3,50%)", String.raw`\((([^,]+,){3}|(\s*\S+[\s/]+){3}).*?%` ], [ "rgb(1 2 3 / 1)", "^[^,]+$" ], [ "hsl(1turn, 2%, 3%)", "deg|g?rad|turn" ] ].map(e => !CSS.supports("color", e[0]) && e[1]).filter(Boolean).join("|") || null;
          RX_UNSUPPORTED && (RX_UNSUPPORTED = new RegExp(RX_UNSUPPORTED, "i"));
        }
        this.colorize();
        ...
        cm.state.highlight.set = (time, fn) => {
          ...
          timerInvisible || (timerInvisible = setTimeout(colorizeInvisible, time));
        };
      }
      colorize() {
        colorizeAll(this);
    ...
    function colorizeAll(state) {
      ...
      if (viewFrom > 0 || viewTo < cm.doc.size) {
        ...
        timerInvisible || (timerInvisible = cmHighlightWorkers.has(cm) && setTimeout(colorizeInvisible, 100));
      }
    }
    ...
    function colorizeChanges(state, changes) {
      ...
      if (postponed.length) {
        ...
        timerChanges || (timerChanges = setTimeout(colorizeChangesLater));
      }
    }
    ...
    function colorizeLineViaStyles(state, line, lineHandle) {
      ...
      e: for (let v, spanState, marker, start, end, len, hex, funcType, i = 1; i + 1 < stylesLen; i += 2) {
        ...
        if (markedSpans) {
          spansSorted || (spansSorted = markedSpans = markedSpans.sort((a, b) => a.from - b.from));
        ...
        const parsedColor = func ? (v = color.slice(len + 1, -1).trim()) && (!funcType || pe.parseColorFunc(funcType, funcType === 2 ? v : v.toLowerCase())) : pe.default.parse(color, end - start, hex);
        if (parsedColor || func) {
          if (spanState) {
            ++spansZombies;
          ...
          marker.color = color;
          marker.css = le.SWATCH_PROP + ":" + (parsedColor && (_ = RX_UNSUPPORTED) != null && _.test(color) ? parsedColor.toString(2) : color);
    ...
    function openPopupForCursor(state) {
      ...
      doOpenPopup(state, line, markerStart != null ? markerStart : ch, marker);
    }
    ...
    function doOpenPopup(state, line, ch, marker) {
      ...
      const color = (marker == null ? void 0 : marker.color) || data.defaultColor;
    ...
    function makePalette({cm, options}) {
      ...
      if (palette.size > 1 || nums && nums.length > 1) {
        var _;
        const old = new Map((_ = options.popup.palette) == null ? void 0 : _.map(el => [ el.__color, el ]));
