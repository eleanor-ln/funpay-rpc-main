...
this["apply.js"] !== 1 && (() => {
  ...
  const apiHandler = {
    ...
    apply: async (path, ...args) => {
      ...
      for (let res, err, retry = 0; retry < 2; retry++) {
        try {
          res = await new Promise((resolve, reject) => chrome.runtime.sendMessage(msg, res2 => (err = chrome.runtime.lastError) ? reject(err) : resolve(res2)));
          if (res) {
  ...
  chrome.runtime.onMessage.addListener(({data, multi, TDM, broadcast}, sender, sendResponse) => {
    data.method === "backgroundReady" && (bgReadySignal == null || bgReadySignal(!0));
    sender.TDM = TDM;
  ...
  const ownId = runtime.getURL("").split("/")[2];
  const isXml = document instanceof XMLDocument;
  const wrappedDoc = style_injector_FF && document.wrappedJSObject || document;
  const assDoc = style_injector_FF && !IDBIndex.prototype.getAllRecords ? wrappedDoc : document;
  ...
  function addAllElements() {
    if (list.length) {
      if (!checkCSP) {
        checkCSP = evt => {
          let sent, src, u, what;
          if (evt.isTrusted && (u = evt.blockedURI) && /(default|style|img|font)-src/.test(what = evt.violatedDirective) && !(sent != null ? sent : sent = new Set).has(what = u + ` (${what})`) && (!(src = evt.sourceFile) || /^(\w+-)?extension$/.test(src))) for (const style of list) if (nonCommentIncludes(style.code, u === "inline" ? u = "data:" : u)) {
  ...
  function replaceAss(readd) {
    ...
    const res = assDoc !== document ? cloneInto([], wrappedDoc) : [];
  ...
  function apply({cfg, sections}, isReplace) {
    ...
    if (isEnabled) {
      ...
      style_injector_FF && assV2 && old && !isReplace && sections.length === 1 && ~assIndexOf(assDoc[kAss], old.el) ? assDoc[kAss] = assDoc[kAss] : restoreOrder();
    }
    emitUpdate();
  }
  ...
  function createStyle(style) {
    ...
    if (ass) {
      var o, r;
      exposeStyleName || (id = (r = randomIds[o = id]) != null ? r : randomIds[o] = Math.random().toString(36).slice(2));
      id = MEDIA + id;
  ...
  function initAss() {
    if (!assIndexOf) {
      ...
      assIndexOf = style_injector_FF ? (arr, {media: {mediaText: id}}) => {
  ...
  function initCreationDoc(style) {
    creationDoc = style_injector_FF && Event.prototype.getPreventDefault ? document : wrappedDoc;
  ...
  function restoreOrder(mutations) {
    ...
    if (el) if (ass) {
      ...
      for (let len = list.length, base = ass.length - len, i = 0; i < len; i++) if (base < 0 || (style_injector_FF ? ass[base + i].media.mediaText !== list[i].el.media.mediaText : ass[base + i] !== list[i].el)) {
        bad = !0;
        break;
      }
  ...
  function updateConfig(cfg) {
    ...
    ffCsp = !nonce && style_injector_FF && isSecureContext;
  ...
  const isUnstylable = style_injector_FF && isXml;
  const clone = !style_injector_FF && (val => typeof val == "object" && val ? JSON.parse(JSON.stringify(val)) : val);
  ...
  const instanceId = (style_injector_FF || !CSS.supports("top", "1ic")) && Math.random() || 0;
  const xoEventId = `${instanceId || Math.random()}`;
  const NAV_ID = "url:" + runtime.id;
  const navHub = style_injector_FF ? global : global[NAV_ID] = new EventTarget;
  const navHubParent = isFrameNoUrl && (style_injector_FF ? apply_parent : apply_parent[NAV_ID]) || null;
  let mqDark;
  let offscreen;
  let port;
  let throttled;
  ...
  style_injector_FF || (global[Symbol.for("xo")] = (el, cb) => {
  ...
  async function init() {
    if (isUnstylable) return API.util.styleViaAPI({
      method: "styleApply"
    });
    let data;
    (data = isFrameNoUrl && !style_injector_FF && clone(apply_parent[apply_parent.Symbol.for(SYM_ID)])) ? await new Promise(onFrameElementInView) : data = getStylesViaXhr();
    if (!runtime.id) return apply_selfDestruct();
  ...
  function updateDisableAll() {
    isUnstylable ? API.util.styleViaAPI({
  ...
  function updateCount() {
    ...
    if (!(TDM < 0)) {
      ...
      if (isUnstylable) API.util.styleViaAPI({
        method: "updateCount"
  ...
  function onFrameElementInView(cb) {
    ...
    (offscreen != null ? offscreen : offscreen = []).push(cb);
  }
