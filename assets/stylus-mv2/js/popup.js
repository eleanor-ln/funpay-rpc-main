...
(self.webpackChunkStylus = self.webpackChunkStylus || []).push([ [ "popup" ], {
  2292(_, ee, oe) {
    ...
    const EntryClick = {
      ...
      ".menu-button": Object.assign((event, entry) => Ee.openMenu(entry), {
        btn: 2
      }),
      [selEdit]: openEditor
    };
    const GlobalClick = {
      ...
      [selOptions]: (event, entry, button) => Ie.openDashboard(null, button === 2, close, {
        windowId: Ce.windowId
      })
    };
    const styleFinder = {};
    const tSideHint = "\n" + Se.t("popupSidePanelOpenHint");
    ...
    document.querySelector(selFinder).on("split-btn", async e => {
      styleFinder.on || await oe.e("popup_search_js").then(oe.bind(oe, 5517));
      styleFinder.inSite(e);
    });
    document.querySelector(selManager).title += "\n<Shift>: " + Se.t("popupManageSiteStyles");
    document.querySelector(selManager).on("split-btn", openManager);
    he.FIREFOX && le.isTouch && xe.installed.on("click", Se.NOP);
    fe.subscribe(Object.keys(sideTitleMap), (id, alwaysSidebar) => {
      typeof alwaysSidebar == "number" && (alwaysSidebar = alwaysSidebar === 0);
      id === pSideEditor && updateTitle(ue.template.style.$(selEdit), alwaysSidebar);
      for (const el of document.querySelectorAll(sideTitleMap[id])) updateTitle(el, alwaysSidebar);
    }, !0);
    pe.onMessage.set(({method, reason, style}) => {
      var _;
      if (!Ce.tabUrl) return;
      const del = method === "styleDeleted";
      const busy = (del || method === "styleAdded" || method === "styleUpdated") && !/^editPreview/.test(reason) && xe.updateStyleEntry(style.id, del);
      busy && ((_ = styleFinder.on) == null || _.call(styleFinder, method, style.id, busy));
    });
    let hideContextMenu;
    window.on("auxclick", clickRouter, !0);
    window.on("click", clickRouter, !0);
    if (Ie.browserSidebar) {
      window.on("contextmenu", evt => hideContextMenu ? evt.preventDefault() : clickRouter(evt, 2), !0);
      he.FIREFOX && window.on("keydown", evt => {
        evt.metaKey || evt.altKey || evt.ctrlKey || evt.key !== (evt.shiftKey ? "F10" : "ContextMenu") || clickRouter(evt, 2);
      }, !0);
    }
    ...
    function nameRouter(event, entry, button) {
      event.altKey || button && this.localName === "input" ? $e.toggleStateInTab([ entry ], null) : button || event.ctrlKey || he.MAC && event.metaKey ? openEditor(0, entry) : button || event.shiftKey || ye.API.styles.toggle(entry.styleId, !entry.styleMeta.enabled);
    }
    async function configure(event, entry, button) {
      if (this.target) openURLandHide.call(this, event); else {
        let mode;
        if (!Se.isSidebar && Ie.browserSidebar && (button || !(mode = fe.__values[pSideConfig]) || mode > 0 && entry.styleMeta.usercssData.vars >= mode)) return Ie.openSidebar(`sidepanel.html?id=${entry.styleId}`, close, {
          tabId: Ce.tabId
        });
        $e.pause(() => ae.configDialog(entry.styleId, entry.getBoundingClientRect().bottom));
      }
    }
    async function openEditor(event, entry, button) {
      const params = entry ? "?id=" + entry.styleId : this.search;
      if (Ie.browserSidebar && (button === 2 || fe.__values[pSideEditor])) return Ie.openSidebar("edit.html" + params, close, {
        tabId: Ce.tabId
      });
      await ye.API.tabs.openEditor(params);
      Se.isSidebar || close();
    }
    function openManager(event, entry, button) {
      event == null || event.preventDefault();
      return Ie.openDashboard(event.shiftKey || event.detail === "site" ? {
        search: Ce.tabUrl,
        searchMode: "url"
      } : {}, button === 2, close, {
        windowId: Ce.windowId
    ...
    async function openStyleFinder(event, entry, button) {
      if (Ie.browserSidebar && (button === 2 || fe.__values[pSideFinder])) return Ie.openSidebar(`popup.html?${pSideFinder}`, close, {
        tabId: Ce.tabId
      });
      this.disabled = !0;
      styleFinder.on || await oe.e("popup_search_js").then(oe.bind(oe, 5517));
    ...
    async function openURLandHide(event) {
      event.preventDefault();
      const tab = await Ie.getActiveTab();
      await ye.API.tabs.open({
  ...
  2950(_, ee, oe) {
    ...
    function toggleStateInTab(list, enable) {
      ...
      for (let el of list) if (el.id || (el = document.getElementById("style-" + el))) {
        ...
        const tabOn = ovr != null ? ovr : siteOn;
        enable !== tabOn && ((ids != null ? ids : ids = {})[style.id] = ovr == null || (ovr ? siteOn : !siteOn) ? enable != null ? enable : !tabOn : null);
      }
  ...
  6153(_, ee, oe) {
    ...
    const isFullscreenPopup = he.MOBILE && innerWidth > screen.availWidth - 200 && innerHeight > screen.availHeight - 200;
    let tabId;
    let tabUrl;
    let tabUrlSupported;
    let windowId;
    ...
    isFullscreenPopup || window !== top || window.on("resize", function onWindowResize() {
      ...
      if (h > prevHeight && document.body.clientHeight > h + 1) {
        ...
        document.body.style.maxHeight = h - (he.CHROME < 125) + "px";
      }
      prevHeight = h;
    });
    !async function init(data, port) {
      data != null || (data = (await ye.clientData).popup || {});
      initPopup(data);
      Ee.showStyles(data);
      $e.initHotkeys(data);
      if (!port) {
        Se.urlParams.has(Ce.pSideFinder) && Ce.openStyleFinder();
        !function connectPort() {
          Ie.ignoreChromeError();
    ...
    Ee.updateStateIcon(fe.isDark);
    fe.onDarkChanged.add(val => Ee.updateStateIcon(val, null));
    ye.subscribe("popup.stylesFirst", (key, stylesFirst) => {
      le.$rootCL.toggle("styles-first", stylesFirst);
      le.$rootCL.toggle("styles-last", !stylesFirst);
    }, !0);
    ye.subscribe("disableAll", (key, val) => {
      Ee.updateStateIcon(null, val);
      document.getElementById("disableAll-label").title = Se.t("masterSwitch") + ":\n" + Se.t(val ? "disableAllStylesOff" : "genericEnabledLabel");
    }, !0);
    async function initPopup({frames, ping0, tab, urlSupported}) {
      ...
      if (tabUrl) {
        ...
        if (isFullscreenPopup || Se.isSidebar) le.$rootCL.add("maximized"); else {
          const kPopupWidth = "popupWidth";
          ye.subscribe([ kPopupWidth, "popupWidthMax" ], (key, val) => {
            document.body.style[(key === kPopupWidth ? "min" : "max") + "-width"] = Se.clamp(val, 200, 800) + "px";
          }, !0);
        }
        ae.setupLivePrefs();
        ye.__values["popup.toggler.expanded"] && (document.querySelector("#toggler details").open = !0);
        el = document.querySelectorAll("#toggler label")[1];
        el.title = el.title.replace("<", he.MAC ? "<⌥" : "<Alt-");
      ...
      if (ping0) return;
      const isStore = he.FIREFOX ? tabUrl.startsWith("https://addons.mozilla.org/") : he.OPERA ? tabUrl.startsWith("https://addons.opera.com/") : tabUrl.startsWith("https://chrome.google.com/webstore/") || tabUrl.startsWith("https://chromewebstore.google.com/");
      blockPopup();
      if (he.CHROME && isStore || !urlSupported) return;
      for (let t2 = performance.now() + 1e3; performance.now() < t2; ) {
        if (await pe.API.tabs.ping(tabId)) {
          blockPopup(!1);
          return;
        }
        if (tab.status === "complete" && (he.CHROME || tab.url !== "about:blank")) break;
        await Se.sleep0();
        tab = await Ie.getActiveTab();
      }
      let info;
      if (he.CHROME) info = ue.template.unreachableInfo; else if (he.FIREFOX) {
        info = document.querySelector(Ce.selUnstylable);
        info.$("a").title = [ !isStore && Se.t("unreachableCSP", Se.t("optionsAdvancedPatchCsp")), isStore && Se.t(he.FIREFOX >= 59 ? "unreachableAMOHint" : "unreachableMozSiteHintOldFF"), he.FIREFOX >= 60 && Se.t("unreachableMozSiteHint") ].filter(Boolean).join("\n");
        info.$("span").textContent = Se.t("unreachableAMO");
      }
      let fileHint = tabUrl.startsWith("file:") ? (!he.FIREFOX || he.FIREFOX >= 153) && "unreachableFileHint" : he.OPERA && "unreachableOpera";
      if (fileHint) {
        fileHint = Se.t(fileHint);
        he.FIREFOX && (fileHint = fileHint.replace("chrome://extensions", "about:addons"));
  ...
  446(_, ee, oe) {
    ...
    ee.openMenu = async entry => {
      ...
      const ovr = style.tabOvr;
      elMatched.textContent = (_ = style.matchedOvrs) != null ? _ : await pe.API.styles.matchOverrides(id, he.tabUrl);
      let prevRule;
      ...
      url && Object.assign(elHome, {
        href: url,
        title: fe.t("externalHomepage") + (ye.FIREFOX ? "" : "\n" + url)
      });
      menuCL.add("measure");
    ...
    function initMenu() {
      ...
      (btnEdit = menu.$('[data-cmd="edit"]')).onclick = Se.openEditor;
      btnEdit.title = "<F2>: " + fe.t("styleSitesPopupEdit");
      ...
      for (const el of menu.$$("[data-ovr]")) {
        ...
        i = `<${i}>` + (i ? "" : ", <`>") + ": ";
        elInc.title = `${i}${fe.t("include")}`;
        elExc.title = `${i.replace(/</g, "<Shift-")}${fe.t("exclude")}`;
      }
    }
    function onOvrChanged(evt) {
      ...
      (rule ? pe.API.styles.toggleSiteOvr(id, rule, isInc, val) : pe.API.styles.toggleTabOvrMany(he.tabId, {
        [id]: val ? isInc : null
      })).catch(fe.NOP);
    }
    ee.menu = menu;
  ...
  2329(_, ee, oe) {
    ee.createWriterElement = (frame, index) => {
      ...
      const url = Ce.tabUrlSupported || frameId ? frame.url.split("#")[0] : "https://www.example.com/abcd";
      ...
      if (isAbout) {
        ...
        el = (url.startsWith(he.ownRoot) ? makeExtCrumbs : makeWebCrumbs)(crumbs, url);
    ...
    ee.updateStyleEntry = async (id, del) => {
      ...
      const inMenu = id === Ee.menu.styleId && Ee.menu.isConnected;
      const res = !del && await pe.API.styles.getByIdInTab(id, Ce.tabId, inMenu);
      if (res) {
        const el = createStyleElement(Object.assign(res.style, res), entry);
        el.isConnected || installed.append(el);
        reSort();
        inMenu && Ee.openMenu(el);
      } else {
        if (entry) {
          entry.remove();
          reSort();
    ...
    const xo = new IntersectionObserver(results => {
      for (const {target: $name, boundingClientRect: r} of results) {
        ...
        $name.title = [ $name.scrollWidth > r.width + 1 && $name.textContent, style.sloppy && Se.t("styleNotAppliedRegexpProblemTooltip"), style.excluded && Se.t("styleNotAppliedExcluded", Se.t("styleSitesExclude")), style.excludedScheme && Se.t(`styleNotAppliedScheme${Se.capitalize(style.preferScheme)}`), style.included && Se.t("styleForceApplied", Se.t("styleSitesInclude")), tabOvr ? Se.t("styleForceAppliedTab") : tabOvr === !1 && Se.t("styleNotAppliedExcludedTab"), style.incOvr && Se.t("styleNotAppliedOverridden", Se.t("styleSitesInclude")) ].filter(Boolean).join("\n") || "";
      }
    });
    ...
    function makeExtCrumbs(crumbs, url) {
      ...
      const page = url.slice(he.ownRoot.length, url.indexOf(".html"));
      crumbs.push(makeCrumb(key, all + ".+", EXT_NAME, EXT_NAME, !0));
      return makeCrumb(key, `${all}[^/]+/${Se.stringAsRegExpStr(page)}.*`, EXT_NAME, page + ".*");
    }
    function makeWebCrumbs(crumbs, url) {
      ...
      return makeCrumb("url-prefix", url, "", Se.clipString(tail) || Se.t("writeStyleForURL"));
    }
    ...
    function createStyleElement(style, entry) {
      ...
      const elEmpty = oldEntry == null ? void 0 : oldEntry.$(".i-empty");
      const elSloppy = oldEntry == null ? void 0 : oldEntry.$(".regexp-problem-indicator");
      const elCsp = oldEntry == null ? void 0 : oldEntry.$(".csp-problem-indicator");
      ...
      if (hasVars) if (ucd) cfg.title = Se.t("configureStyle"); else {
        cfg.href = url;
        cfg.target = "_blank";
        cfg.title = Se.t("configureStyleOnHomepage") + "\n" + url;
        cfg.$("i").className = "i-external";
      } else cfg.hidden = !0;
      !Se.isSidebar && ye.__values[$e.pSideConfig] && (cfg.title += $e.tSideHint);
      if (frameUrl) {
        const sel = "span.frame-url";
        (entry.$(sel) || name.insertBefore(le.$create(sel), name.lastChild)).title = frameUrl;
      }
      empty ? elEmpty || entry.$(".main-controls").append(ue.template.errEmpty.cloneNode(!0)) : elEmpty == null || elEmpty.remove();
      csp ? renderErrCsp(entry, elCsp, csp) : elCsp == null || elCsp.remove();
      sloppy ? elSloppy || (entry.$(".main-controls").appendChild(ue.template.errRegexp.cloneNode(!0)).onShowNote = onShowNotePartial) : elSloppy == null || elSloppy.remove();
      oldEntry && xo.unobserve(name);
      xo.observe(name);
      return entry;
    }
    function renderErrCsp(entry, elCsp, csp) {
      titleCSP != null || (titleCSP = `${Se.t("styleAssetsCSP", Se.t("optionsAdvancedPatchCsp"))}\n<pre>`);
      elCsp != null || (elCsp = entry.$(".main-controls").appendChild(ue.template.errCsp.cloneNode(!0)));
      elCsp.title = titleCSP + Object.keys(csp).map(k => Se.clipString(k, 50)).join("\n") + "</pre>";
    ...
    function onShowNoteCsp({_buttons: el}) {
      el.append(le.$create("button#options-btn", Se.t("openOptions")));
    }
    function onShowNotePartial({_body: el}) {
      el.append("\n\n", ue.sanitizeHtml(Se.t("styleRegexpPartialExplanation")));
      if (el = el.$("a")) {
        el.href = "https://developer.mozilla.org/docs/Web/CSS/@document";
        fe.CHROME && (el.title = el.href);
      }
    }
  ...
  _.O(0, [ "color", "dlg_config-dialog_css-css_dom-error_css-css_global-dark_css-css_global_css-css_onoffswitch_cs-1e56de" ], () => _(_.s = 6153));
  _.O();
} ]);